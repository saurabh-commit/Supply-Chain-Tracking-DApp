...
// Inherit the Ownable.sol smart contract to leverage the modifiers defined in it
...
contract SupplyChain is Ownable, FarmerRole, DistributorRole, RetailerRole, ConsumerRole{
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, "Failed to verify the caller"); 
    _;
  }

  // Define a modifier that checks if the paid amount is sufficient to cover the price
  modifier paidEnough(uint _price) { 
    require(msg.value >= _price, "Paid amount not enough"); 
    _;
  }
    ...
    string  memory _productNotes) public onlyFarmer
  {
    ...
    items[_upc].productPrice = uint(0);
    items[_upc].itemState = State.Harvested;
    items[_upc].distributorID = address(0);  // Metamask-Ethereum address of the Distributor
    items[_upc].retailerID = address(0); // Metamask-Ethereum address of the Retailer
    items[_upc].consumerID = address(0); // Metamask-Ethereum address of the Consumer
    // Increment sku
    sku = sku + 1;
  ...
  function buyItem(uint _upc) public payable 
    ...
    originFarmerAddressPayable.transfer(items[_upc].productPrice);
    // emit the appropriate event
    emit Sold(_upc);
  }
  ...
  function receiveItem(uint _upc) public 
    // Call modifier to check if upc has passed previous supply chain stage
    shipped(_upc)
    // Call function to check if purchaser is in fact a Retailer:
    onlyRetailer
  ...
  function purchaseItem(uint _upc) public 
    // Call modifier to check if upc has passed previous supply chain stage
    received(_upc)
    // Call function to check if purchaser is in fact a Consumer:
    onlyConsumer
    // Access Control List enforced by calling Smart Contract / DApp
    {
